...
class EnrichmentService {
     ...
     */
    async enrichLeadData(leadId, cnpj) {
        let enrichedData = {};
        let usedFallback = false;
        if (cnpj) {
            try {
                const cleanCnpj = cnpj.replace(/\D/g, '');
                const response = await axios_1.default.get(`https://brasilapi.com.br/api/cnpj/v1/${cleanCnpj}`, { timeout: 5000 });
        ...
        const currentLead = await prisma_1.prisma.lead.findUnique({
            where: { id: leadId },
     ...
     */
    calculateScoreFromCapital(capital) {
        const val = capital || 0;
        if (val > 1000000)
            return 950;
        if (val > 150000)
     ...
     */
    calcularQualidadeEcom(capitalSocial, plataforma) {
