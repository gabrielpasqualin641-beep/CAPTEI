...
const spintax_1 = require("../lib/spintax");
 ...
 */
function findNextNode(currentNodeId, edges, nodes, sourceHandle) {
 ...
 */
async function handleSendNode(node, lead, instanciaKey) {
    const data = node.data;
    const textWithSpintax = (0, spintax_1.parseSpintax)(data.message || '');
    const mensagem = renderMessage(textWithSpintax, lead);
    const whatsappNumber = normalizePhone(lead.telefone);
    console.log(`[WorkflowWorker] 📤 sendNode "${data.label}" → Lead: ${lead.nome} (${whatsappNumber})`);
    ...
    if (mediaType !== 'text' && data.mediaUrl) {
        ...
        await evolution_1.evolutionApi.sendMedia(instanciaKey, whatsappNumber, data.mediaUrl, mediaType, mensagem, 1200);
    }
    else if (mediaType === 'text' && data.buttons && data.buttons.length > 0) {
        // Envia mensagem com botões interativos nativos do WhatsApp
        await evolution_1.evolutionApi.sendButtons(instanciaKey, whatsappNumber, mensagem, data.buttons, 1200);
    }
    else {
        ...
        await evolution_1.evolutionApi.sendText(instanciaKey, whatsappNumber, mensagem, 1200);
    }
 ...
 */
async function handleAINode(node, lead) {
    const data = node.data;
    console.log(`[WorkflowWorker] 🤖 aiNode "${data.label}" — Modelo: ${data.model ?? 'llama-3'}`);
    // Busca a última mensagem recebida do lead
    const lastIncomingInteraction = await prisma_1.prisma.interacao.findFirst({
        where: { lead_id: lead.id, tipo: 'recebido' },
    ...
    const classification = await aiService_1.AIService.classifyResponse(textToClassify, data.systemPrompt);
 ...
 */
async function handleCRMFilterNode(node, lead) {
    const data = node.data;
    const targetStatus = data.trueStatus ?? 'Respondeu';
    console.log(`[WorkflowWorker] 🔀 crmFilterNode "${data.label}" | Status do lead: "${lead.status}" | IA Intenção: "${lead.ia_intencao}" | Alvo: "${targetStatus}"`);
    // Condição é atendida se o status bate, ou se o alvo é "Respondeu" e a intenção da IA foi classificada como positivo
    const conditionMet = lead.status === targetStatus ||
        (targetStatus === 'Respondeu' && lead.ia_intencao === 'positivo');
    console.log(`[WorkflowWorker]    Resultado: ${conditionMet ? '✅ Verdadeiro' : '❌ Falso'}`);
    return {
...
async function processWorkflowStep(job) {
    const { execucaoId, workflowId, leadId, currentNodeId, incomingHandle } = job.data;
    ...
    const execucao = await (0, prisma_1.bypassTenantIsolation)(() => prisma_1.prisma.workflowExecucao.findUnique({
        where: { id: execucaoId },
    }));
    if (!execucao) {
    ...
    return prisma_1.tenantStorage.run({ userId: execucao.userId, isSystem: false }, async () => {
        if (execucao.status === 'pausado' || execucao.status === 'concluido') {
            console.log(`[WorkflowWorker] ⏸ Execução ${execucaoId} está "${execucao.status}". Job ignorado.`);
            return;
        }
        ...
        const [workflow, lead] = await Promise.all([
            ...
            prisma_1.prisma.lead.findUnique({ where: { id: leadId } }),
        ]);
        if (!workflow)
        ...
        console.log(`[WorkflowWorker] 🧩 Nó tipo "${currentNode.type}" — "${currentNode.data.label ?? '—'}"`);
        // ── 6. Executa a lógica do nó ──────────────────────────────────────────────
        // Busca a instância WhatsApp ativa (reutiliza lógica do queue.ts existente)
        const instancia = await prisma_1.prisma.instancia.findFirst({
        ...
        switch (currentNode.type) {
            case 'sendNode': {
                const result = await handleSendNode(currentNode, lead, instanciaKey);
                outputHandle = result.outputHandle;
                nodeDelayMs = result.delayMs;
                break;
            }
            case 'aiNode': {
                const result = await handleAINode(currentNode, lead);
                outputHandle = result.outputHandle;
                nodeDelayMs = result.delayMs;
                break;
            }
            case 'crmFilterNode': {
                ...
                const freshLead = await prisma_1.prisma.lead.findUnique({
                    where: { id: leadId },
                    select: { status: true, ia_intencao: true },
                });
                const result = await handleCRMFilterNode(currentNode, freshLead ?? { status: lead.status, ia_intencao: lead.ia_intencao });
            ...
            default: {
                console.warn(`[WorkflowWorker] ⚠️  Tipo de nó desconhecido: "${currentNode.type}". Pulando.`);
                outputHandle = 'output';
                break;
            }
        }
        ...
        if (!nextNode) {
            ...
            console.log(`[WorkflowWorker] 🏁 Fim do fluxo para lead ${lead.nome}. Execução concluída.`);
            await prisma_1.prisma.workflowExecucao.update({
                where: { id: execucaoId },
                data: { status: 'concluido', bullmq_job_id: null },
            });
        ...
        const jitter = isFollowUp
            ? (0, workflow_queue_1.randomJitter)(JITTER_MIN_MS, JITTER_MAX_MS)
            : (0, workflow_queue_1.randomJitter)(500, 3_000); // jitter curto para steps rápidos
        const totalDelay = baseDelay + jitter;
        console.log(`[WorkflowWorker] ⏭  Próximo nó: "${nextNode.id}" (${nextNode.type}) | Delay: ${Math.round(totalDelay / 1000)}s (base: ${Math.round(baseDelay / 1000)}s + jitter: ${Math.round(jitter / 1000)}s)`);
        ...
        const nextJobId = await (0, workflow_queue_1.enqueueNextStep)({
            jobData: nextJobData,
            delayMs: baseDelay,
            jitterMs: jitter,
        });
 ...
 */
async function trackLeadActiveJob(leadId, jobId) {
    const key = (0, workflow_types_1.LEAD_ACTIVE_JOBS_KEY)(leadId);
    await queue_2.connection.sadd(key, jobId);
    await queue_2.connection.expire(key, 7 * 24 * 60 * 60); // 7 dias
}
/**
 * Remove um job ID do Set Redis de jobs ativos do lead.
 */
async function removeLeadActiveJob(leadId, jobId) {
    const key = (0, workflow_types_1.LEAD_ACTIVE_JOBS_KEY)(leadId);
    await queue_2.connection.srem(key, jobId);
}
// ─── Instância do Worker ──────────────────────────────────────────────────────
exports.workflowWorker = new bullmq_1.Worker(workflow_queue_1.WORKFLOW_QUEUE_NAME, processWorkflowStep, {
...
exports.workflowWorker.on('error', (err) => {
