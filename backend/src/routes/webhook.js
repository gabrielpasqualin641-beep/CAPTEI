...
function normalizePhoneForLookup(remoteJid) {
...
// ─────────────────────────────────────────────────────────────────────────────
router.post('/evolution/:instancia', auth_1.systemBypassMiddleware, async (req, res) => {
    // Garante bypass explícito de tenant isolation (operando como serviço de sistema/webhook)
    return (0, prisma_1.bypassTenantIsolation)(async () => {
        try {
            ...
            if (isOptOut) {
                ...
                const canceledEnvios = await prisma_1.prisma.envio.updateMany({
            ...
            if (canceledEnvios.count > 0) {
                console.log(`[Webhook] ⏹  ${canceledEnvios.count} envio(s) agendado(s) cancelados para "${lead.nome}"`);
            }
 ...
 */
async function pauseWorkflowExecutions(leadId, leadNome) {
    // ── Busca job IDs ativos no Redis ──────────────────────────────────────
    const redisKey = (0, workflow_types_1.LEAD_ACTIVE_JOBS_KEY)(leadId);
    const activeJobIds = await queue_1.connection.smembers(redisKey);
    if (activeJobIds.length === 0) {
        console.log(`[Webhook] Nenhum job de workflow ativo para "${leadNome}".`);
    }
    else {
        console.log(`[Webhook] 🔴 Cancelando ${activeJobIds.length} job(s) de workflow para "${leadNome}":`, activeJobIds);
        // ── Remove cada job da fila BullMQ ────────────────────────────────────
        const cancelResults = await Promise.allSettled(activeJobIds.map(async (jobId) => {
            try {
                const job = await workflow_queue_1.workflowQueue.getJob(jobId);
                if (!job) {
                ...
                if (jobState === 'delayed' || jobState === 'waiting') {
                    ...
                    console.log(`[Webhook]   ⚠️  Job ${jobId} está em execução. Será interrompido pelo worker no próximo checkpoint.`);
                }
            }
            catch (err) {
        ...
        // ── Limpa o Set Redis ──────────────────────────────────────────────────
        await queue_1.connection.del(redisKey);
    }
    // ── Marca WorkflowExecucao como 'pausado' no banco ────────────────────────
    const { count } = await prisma_1.prisma.workflowExecucao.updateMany({
        where: {
            lead_id: leadId,
    ...
    if (count > 0) {
        console.log(`[Webhook] ✅ ${count} execução(ões) de workflow pausadas para "${leadNome}" — robô interrompido para atendimento humano.`);
    }
}
