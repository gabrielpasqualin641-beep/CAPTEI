...
router.get('/scrape-progress', authenticate, (req, res) => {
  const { jobId } = req.query;
  if (!jobId || typeof jobId !== 'string') return res.json({ progress: null });
  const data = progressMap.get(jobId);
  return res.json({ progress: data || null });
});

// ── GET /national-status — progresso da varredura nacional ───────────────────
router.get('/national-status', authenticate, async (req, res) => {
  ...
  try {
    const varredura = await prisma.varreduraJob.findUnique({
      where: { jobGroupId },
    });
    if (!varredura) {
...
router.delete('/national-cancel', authenticate, async (req, res) => {
  const { jobGroupId } = req.query;
  if (!jobGroupId || typeof jobGroupId !== 'string') {
    return res.status(400).json({ error: 'jobGroupId é obrigatório.' });
  }
  try {
    ...
    const waitingJobs = await nationalScrapeQueue.getJobs(['waiting', 'delayed']);
...
const isBlacklistedCompany = (name: string): boolean => {
  const lower = name.toLowerCase();
  return COMPANY_BLACKLIST.some(word => lower.includes(word));
};

const detectPlatform = (html: string, url: string): string => {
  const htmlLower = html.toLowerCase();
  if (url.includes('nuvemshop') || htmlLower.includes('nuvemshop.com.br')) return 'Nuvemshop';
  if (url.includes('myshopify') || htmlLower.includes('shopify.') || htmlLower.includes('cdn.shopify.com')) return 'Shopify';
  if (htmlLower.includes('/wp-content/plugins/woocommerce')) return 'WooCommerce';
  if (url.includes('lojas.tray.com.br') || htmlLower.includes('tray.com.br')) return 'Tray';
  if (htmlLower.includes('vtex')) return 'VTEX';
  if (htmlLower.includes('loja integrada') || htmlLower.includes('lojaintegrada.com.br')) return 'Loja Integrada';
  return 'Outra/Customizada';
};

const extractContact = (html: string) => {
...
const mapOSMTagsEcommerce = (termo: string) => {
  ...
  if (t.includes('eletrônico') || t.includes('eletronico')) return ['electronics', 'mobile_phone', 'computer'];
  if (t.includes('pet')) return ['pet', 'pet_grooming'];
  if (t.includes('joia') || t.includes('acessório')) return ['jewelry', 'watches'];
  if (t.includes('móvel') || t.includes('decoração') || t.includes('movel') || t.includes('decoracao')) return ['furniture', 'interior_decoration', 'carpet', 'curtain'];
  if (t.includes('cosmético') || t.includes('beleza') || t.includes('perfume') || t.includes('cosmetico')) return ['cosmetics', 'perfumery', 'beauty'];
  if (t.includes('esporte') || t.includes('fitness')) return ['sports', 'outdoor', 'fitness'];
  if (t.includes('alimento') || t.includes('comida') || t.includes('gourmet')) return ['deli', 'bakery', 'confectionery', 'organic', 'health_food'];
  if (t.includes('brinquedo') || t.includes('infantil') || t.includes('bebê') || t.includes('bebe'))    return ['toys', 'baby_goods'];
};
/**
 * runEcommerceScrape — Função pura exportada.
 * Executa a busca e scraping de e-commerces.
 */
export async function runEcommerceScrape(
  nicho: string,
  cidade: string,
  jobId?: string,
): Promise<any[]> {
  ...
  if (jobId) progressMap.set(jobId, { status: 'Analisando sites encontrados...', total: uniqueUrls.length, processados: 0 });
  ...
  for (const siteUrl of uniqueUrls) {
    if (jobId) progressMap.set(jobId, { status: `Raspando site ${processados + 1} de ${uniqueUrls.length}`, total: uniqueUrls.length, processados });
    processados++;
          ...
          if (contatoUrl) {
            ...
            try {
              ...
              if (contatoRes && contatoRes.data) {
                ...
                if (!instagram && c.instagram) instagram = c.instagram;
              }
            } catch (e) {}
          }
        }
    ...
    if (isBlacklistedCompany(nomeDaLoja)) continue;

    const nomeNormalizado = nomeDaLoja.toLowerCase().trim();
    if (seenNames.has(nomeNormalizado)) continue;
    seenNames.add(nomeNormalizado);

    if (phone && email) extractionStatus = '✅ Completo';
    else if (phone || email) extractionStatus = '⚠️ Parcial';
...
// ─────────────────────────────────────────────────────────────────────────────
router.post('/scrape-ecommerce', authenticate, async (req, res) => {
  try {
    ...
    if (!cidade || cidade.trim() === '') {
      ...
      for (let i = 0; i < CAPITAIS_BRASIL.length; i++) {
        ...
        await nationalScrapeQueue.add('national_ecommerce', jobData, {
